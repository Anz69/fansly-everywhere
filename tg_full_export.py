...
async def main():
    ...
    # Pre-fetch all dialogs to populate Telethon entity cache (required for MTProto send_file)
    log("pre-fetching all dialogs for entity cache...")
    try:
        _prefetch = await client.get_dialogs(limit=500)  # bounded — avoids OOM on huge accounts
        log(f"entity cache: {len(_prefetch)} dialogs loaded")
        del _prefetch
    except Exception as _pfe:
        warn(f"pre-fetch dialogs: {_pfe}")
    ...
    async for dialog in client.iter_dialogs():
        ...
        if _dtype == "Private":
            ...
            _msg_limit = args.max_msgs_group   # include ALL groups, no member-count limit
        elif _dtype == "Channel":
            # Include only channels created by this user (creator flag)
